...
# ---------- leave_fullscreen_self / _self_still_fullscreen ----------
# CLAUDE/NOTES/wm-quirks.md#workspace-switch-fullscreen-invisible — see
# leave_fullscreen_self's own docstring for the why. _self_still_fullscreen
# is the pure tree-walking half (same split as _stale_self_marks below);
# leave_fullscreen_self is the thin IPC-polling wrapper around it.

def _sway_tree_with_marked_leaf(mark, fullscreen_mode, id_=60):
    return Con({
        "id": 1, "type": "root",
        ...
        "nodes": [{
            ...
            "nodes": [{
                ...
                "focused": False, "marks": [mark], "pid": 1,
...
def test_self_still_fullscreen_true_when_marked_and_fullscreen():
    import os
    mark = f"{SWAY_MARK_PREFIX}{os.getpid()}"
    tree = _sway_tree_with_marked_leaf(mark, fullscreen_mode=1)

    assert sway_module._self_still_fullscreen(tree, mark) is True


def test_self_still_fullscreen_false_once_cleared():
    import os
    mark = f"{SWAY_MARK_PREFIX}{os.getpid()}"
    tree = _sway_tree_with_marked_leaf(mark, fullscreen_mode=0)

    assert sway_module._self_still_fullscreen(tree, mark) is False


def test_self_still_fullscreen_false_when_mark_not_found():
    tree = _sway_tree_with_marked_leaf("some_other_mark", fullscreen_mode=1)

    assert sway_module._self_still_fullscreen(tree, f"{SWAY_MARK_PREFIX}999") is False


def test_sway_leave_fullscreen_self_returns_once_confirmed(monkeypatch):
    """No timeout needed here — the fake tree already reports
    fullscreen_mode=0 on the very first poll, so the loop never sleeps.
    """
    import os
    mark = f"{SWAY_MARK_PREFIX}{os.getpid()}"
    tree = _sway_tree_with_marked_leaf(mark, fullscreen_mode=0)
    ...
    provider = SwayProvider(conn=conn)
    slept = []
    monkeypatch.setattr(sway_module.time, "sleep", lambda s: slept.append(s))

    provider.leave_fullscreen_self()

    assert conn.commands == [f"[con_mark={mark}] fullscreen disable"]
    assert slept == []


def test_sway_leave_fullscreen_self_gives_up_after_timeout(monkeypatch):
    """If the marked window never actually clears fullscreen_mode (or
    the polled get_tree() never catches up), the loop gives up after
    LEAVE_FULLSCREEN_TIMEOUT_SECONDS instead of hanging forever. time
    itself is faked (a monotonic clock advanced by the mocked sleep) so
    this test doesn't actually burn the real timeout duration.
    """
    import os
    mark = f"{SWAY_MARK_PREFIX}{os.getpid()}"
    tree = _sway_tree_with_marked_leaf(mark, fullscreen_mode=1)  # never clears
    conn = FakeConnection(tree=tree)
    provider = SwayProvider(conn=conn)
    fake_now = [0.0]
    monkeypatch.setattr(sway_module.time, "monotonic", lambda: fake_now[0])
    monkeypatch.setattr(sway_module.time, "sleep", lambda s: fake_now.__setitem__(0, fake_now[0] + s))

    provider.leave_fullscreen_self()  # must return, not hang

    assert conn.commands == [f"[con_mark={mark}] fullscreen disable"]
    assert fake_now[0] >= sway_module.LEAVE_FULLSCREEN_TIMEOUT_SECONDS
