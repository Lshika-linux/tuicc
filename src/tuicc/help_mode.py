...
def resize_help_lines() -> list[str]:
    return [
        ...
        "",
        "F2       enter resize mode on the active module",
        "arrows   resize (w/h) the box, or move it (x/y) — 'm' toggles",
        "         which one arrows currently control",
        "Del      remove the box from the layout (asks y/n first)",
        "Enter    keep the change (in memory) and return to navigation",
        ...
        "F3       save the whole layout as a new preset and switch to it",
        "F4       cycle through every preset that exists",
        "",
        "F1/F3/F4/F6 all work even mid-resize — they commit whatever's",
        "in progress first, same as Enter would.",
        "",
        "Esc  back to menu",
    ]
