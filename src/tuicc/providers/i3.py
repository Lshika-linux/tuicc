...
def _self_focused(tree) -> bool | None:
    ...
    a recorded/synthetic tree) — no pid cross-check needed here unlike
    that function, since matching by the mark string alone is already
    exact. Provider.self_focused() is the thin IPC-issuing wrapper
    around it, same shape as sway.py's own.
    """
