...
from tuicc.modules import sidebar, preview
...
def draw_all(stdscr, layout, boxes, state, selected_id=None):
    for module_box in layout.boxes:
        draw_fn = MODULES.get(module_box.name)
        if draw_fn is None:
            continue
        draw_fn(stdscr, boxes[module_box.name], state, selected_id)
