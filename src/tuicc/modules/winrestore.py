"""Window-layout save/restore module: save the current desktop layout to
a numbered slot, or load/delete a previously saved one — each slot
carrying a user-editable display name (see config.py's winrestore_names).
Box title stays "Sessions" (UI label, independent of this module's own
name) — see winrestore.py's own top docstring for why the underlying
save/restore mechanism itself is no longer session-exact.
...
used. Renaming persists to config.toml's [winrestore] name_<N> keys via
config.set_winrestore_name(), same "type in place, Enter applies"
...
from tuicc.winrestore import capture_session, save_session, load_session, WINRESTORE_DIR
...
# TEMPORARILY DISABLED — see CLAUDE/NOTES/design-decisions.md
# #launcher-placement-mode. The tree-reconstruction restore mechanism
# (TiledRestoreState/advance_tree_build(), winrestore.py) doesn't
# actually work on a real, repeated-app_id-across-groups layout — Rafi's
# own call was to switch "load" off outright rather than ship it
# broken, and rebuild restore later as a replay of the launcher's own
# (separately built, reliable) placement-mode primitives instead —
# nothing about that future work needs this flipped back on first.
# "save" is untouched (still just writes a plain TOML file, no IPC side
# effects, stays forward-compatible with whatever eventually replaces
# this). One-line revert once restore is rebuilt for real.
RESTORE_DISABLED = True

def required_fh(cfg) -> int:
    """One row per slot (always SLOT_COUNT — not user-configurable
    today, see config.py's own WINRESTORE_SLOT_COUNT docstring) plus 2
...
def _session_path(slot):
    return WINRESTORE_DIR / f"{slot}.toml"


def _slot_name(cfg, slot):
    return cfg.winrestore_names.get(slot, f"Slot {slot}")
...
def collapse() -> int | None:
    ...
    caller can reselect "winrestore:<slot>:row" directly — same reason
...
def expanded_preview() -> dict | None:
    ...
    saved layout — None if nothing's expanded, or that slot has
    nothing saved yet. main.py reads this once per frame and threads it
    into RenderContext.winrestore_preview for sidebar.py to render —
    see that field's own docstring for why it goes through main.py
    instead of sidebar.py reaching into this module directly.
    """
    ...
    for entry in load_session(path):
        ...
        if tree is not None:
            app_ids.extend(flatten_leaves(tree))
        app_ids.extend(w["app_id"] for w in entry.get("tiled_flat", []))
        app_ids.extend(w["app_id"] for w in entry.get("floating", []))
        if app_ids:
            preview[entry["target_region"]] = app_ids
    return preview
...
def apply_naming() -> tuple[int, str] | None:
    ...
    "Slot N" default (see config.py's winrestore_names loading), there's
...
def draw(stdscr, box, ctx, module_name):
    ...
    for i, slot in enumerate(range(1, SLOT_COUNT + 1)):
        ...
        is_row_selected = f"winrestore:{slot}:row" == ctx.selected_id
        ...
        for action, cx in _action_positions(x, w):
            ...
            if is_this_expanded:
                is_selected = f"winrestore:action:{slot}:{action}" == ctx.selected_id
...
def nav_items(box, ctx, module_name) -> list[NavItem]:
    ...
    if _expanded_slot is not None:
        ...
        for action, cx in _action_positions(x, w):
            items.append(NavItem(
                id=f"winrestore:action:{_expanded_slot}:{action}",
                rect=(cx, row, len(action), 1),
                focus_target=f"{_expanded_slot}:{action}",
                target_kind="winrestore_action",
            ))
        return items
    ...
    for i, slot in enumerate(range(1, SLOT_COUNT + 1)):
        row = y + 1 + i
        items.append(NavItem(
            id=f"winrestore:{slot}:row",
            rect=(x + 1, row, w - 2, 1),
            focus_target=str(slot),
            target_kind="winrestore_row",
        ))
    return items
...
def handle_row(ctx, item, cfg):
    ...
    # the just-selected "winrestore:N:row" id vanishes from nav_items()'
    ...
    ctx.reselect_item_id = f"winrestore:action:{_expanded_slot}:{ACTIONS[0]}"
    return False, None
...
def handle_action(ctx, item, cfg):
    ...
    if action == "save":
        ...
        ctx.reselect_item_id = f"winrestore:{slot}:row"
        return False, None

    if action == "load":
        if RESTORE_DISABLED:
            ctx.toast_message = "Session restore is temporarily disabled — see README"
        ...
        if overlap:
            return False, {
                ...
                "confirm_text": "Load? (overwrites your layout)",
                "dismiss_after_confirm": False,
                "module": "winrestore",
            }

        # Region-shaped, not spawn-ready yet — see
        # ActionContext.pending_layout_regions' own docstring for why
        # this can't go straight into ctx.restore_queue: frame_update.py
        # drains this into tiled_restore.queued_regions once per frame,
        # running the actual tree-build (real IPC side effects — see
        # winrestore.TreeBuildState/advance_tree_build()) at that point,
        # always past this exact "no confirm needed" decision.
        ctx.pending_layout_regions.extend(entries)
    ...
    if action == "del":
        ...
        return False, {
            ...
            "shell_true": False,
            "confirm_text": f"Delete layout {slot}?",
            "dismiss_after_confirm": False,
            "module": "winrestore",
        }
...
HANDLERS = {
    "winrestore_row": handle_row,
    "winrestore_action": handle_action,
}
