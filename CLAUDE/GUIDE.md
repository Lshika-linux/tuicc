...
**Adding a module** touches `render.py`'s `MODULES` and `NAV_PROVIDERS` dicts only — never `draw_all()`/`collect_nav_items()` themselves (enforced by a comment at the top of `render.py`, not by code). If the module needs a custom `target_kind` (not just the generic `"region"`/`"window"` handled by `actions.py`'s `BASE_HANDLERS`), it registers its own handler(s) into `ACTION_HANDLERS` — either a single `TARGET_KIND` + `handle()` (see `quick_actions.py`, `power_menu.py`) or a `HANDLERS` dict for multiple kinds (see `connectivity.py`: `wifi_browse`/`bluetooth_browse`, which enter level-2 browsing rather than acting directly — the actual per-item connect/pair actions inside that browsing session go through `main.py`'s own `"connectivity_browsing"` `mode_stack` tier, not `ACTION_HANDLERS`). `MODULES` has a second consumer besides `render.py` itself: resize mode's spawn picker (`main.py`) diffs `MODULES.keys()` against `cfg.layout.boxes` to offer only modules not currently placed — a new module is spawnable from inside tuicc for free, no extra wiring.

**Layout boxes are x/y ratios plus, independently per axis, either a `w`/`h` ratio or a fixed `fw`/`fh` cell count** (`layout.py`) — no `right_of`/`below`/`above`/`bottom`/`cols`/`rows`/`fill_to` derivation system anymore (removed; see git history/README if you need the old reasoning). `fw`/`fh` exist for content that needs an exact row/column count rather than a proportion (control's toggle list, connectivity/media/sysmon's tables); `fh_auto` (height only) goes further and recomputes a box's `fh` from its own module's real content every load, instead of a hand-tuned number, until an actual F2 height resize freezes it back to a plain manual `fh`. `compute_boxes()` (`layout_engine.py`) is a flat per-box loop, not a dependency resolver — every box's own configured numbers stay independent of every other box's, though the render-time rect isn't always quite that independent: a fixed box holds its own edge against the terminal instead of getting clipped, a ratio box yields just enough to stop overlapping a fixed neighbor, and two fixed boxes stacked against each other chain flush rather than both claiming the same edge (see `layout.py`'s own module docstring for the exact algorithm). Resizing or moving one box in a preset never changes another box's own configured values.
