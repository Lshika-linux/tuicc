...
`watch()`'s two time parameters serve different purposes: `fallback_seconds` is the actual worst-case staleness bound (also yields on its own if no kernel event arrives in that window, since not every driver reliably calls `power_supply_changed()` for every field this cares about — a pure push design otherwise has no way to recover if the kernel never signals a change). `poll_timeout` only bounds how often the underlying `poll()` call itself returns to recheck `stop_event`/the fallback deadline between real kernel events; it must be `<= fallback_seconds` for that bound to hold (enforced in the function). No packs present at generator-start time returns immediately without yielding — main.py's wiring is expected not to register this as a push domain in that case.
